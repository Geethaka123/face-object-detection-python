...
# Variables to store counts of objects and persons
object_count = 0
person_count = 0
...
while True:
    ...
    # Update person count based on number of faces detected
    ...
    for (x, y, w, h) in faces:
        ...
        if np.sum([roi_gray]) != 0:
            ...
            if label != prev_emotion and label != 'Neutral' and time.time() - last_store_time >= 3:
                timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                sql = "INSERT INTO detections (timestamp, emotion, object, person_count, object_count) VALUES (%s, %s, %s, %s, %s)"
                val = (timestamp, label, prev_object, person_count, object_count)
                cursor.execute(sql, val)
                db.commit()
                prev_emotion = label
                last_store_time = time.time()
    ...
    for classId, confidence, box in zip(classIds.flatten(), confs.flatten(), bbox):
        ...
        if classNames[classId - 1] != prev_object and time.time() - last_store_time >= 3:
            timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            sql = "INSERT INTO detections (timestamp, emotion, object, person_count, object_count) VALUES (%s, %s, %s, %s, %s)"
            val = (timestamp, prev_emotion, classNames[classId - 1], person_count, object_count)
            cursor.execute(sql, val)
            db.commit()
            prev_object = classNames[classId - 1]
            last_store_time = time.time()
